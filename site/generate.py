...
def card(p):
    ...
    return f'<article class="card"><div class="date">{esc(p.get("release_date"))}</div>{badge}<h2><a href="{href}">{esc(display_title(p.get("title")))}</a></h2><p>{esc(display_maker(p.get("maker")))}</p>{f"<p>{esc(talent)}</p>" if talent else ""}<a class="btn" href="{href}">詳細を見る</a></article>'
...
def main():
    ...
    write("index.html", "グラビアDVD新発売情報", '<section class="intro"><h2>最新発売情報</h2><p>グラビアDVDの新発売情報を発売日順に掲載しています。</p>' + top_links + '</section><div class="grid">' + ''.join(card(p) for p in latest) + f'</div><p class="muted">登録商品数：{len(ps)}件</p>')
